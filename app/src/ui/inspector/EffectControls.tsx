...
import {
  type EffectAny, type EffectKind, staticParam, sampleParam, createEffectId,
} from '../../types/project';
...
export function createEffectConfig(kind: EffectKind): EffectAny {
  ...
  switch (kind) {
    ...
    case 'shake':
      return { id, name: 'Shake', kind: 'shake', enabled: true, amountPx: staticParam(3), amountRot: staticParam(0.01), speed: staticParam(8), audioDriven: true, audioAmount: staticParam(2) };
    case 'pulse':
      return { id, name: 'Pulse', kind: 'pulse', enabled: true, baseScaleAdd: staticParam(0), audioAmount: staticParam(0.15), smoothing: staticParam(0.3) };
    case 'strobe':
    ...
    case 'chromaticAberration':
      return { id, name: 'Chromatic Aberration', kind: 'chromaticAberration', enabled: true, amountPx: staticParam(3), angle: staticParam(0), audioDriven: false };
    case 'vignette':
...
const EffectControlsPanel: React.FC<{
  ...
  switch (effect.kind) {
    ...
    case 'shake':
      return <>{paramField('Amount (px)', 'amountPx', 0, 30, 1)}{paramField('Speed', 'speed', 1, 20, 1)}</>;
    case 'pulse':
      return <>{paramField('Audio Amount', 'audioAmount', 0, 1, 0.01)}</>;
    case 'strobe':
      return <>{paramField('Rate (Hz)', 'rateHz', 0.5, 20, 0.5)}{paramField('Duty Cycle', 'dutyCycle', 0, 1, 0.05)}</>;
    case 'blur':
      return <>{paramField('Blur', 'blur', 0, 30, 1)}</>;
    case 'chromaticAberration':
      return <>{paramField('Amount (px)', 'amountPx', 0, 20, 0.5)}</>;
    case 'vignette':
